...
- **Local LLM Integration (Ollama)**: Integrated a local, privacy-first AI companion ("Luna") powered by the `gemma:2b` model via Ollama. 
- **Raw NDJSON Streaming**: Built a custom server-side proxy interception in `src/server.ts` to bypass framework buffering and stream LLM responses directly to the chat interface character-by-character for a snappy, real-time feel.
- **Luna's Persona**: Tuned the system prompt to enforce extremely short, empathetic, and concise responses by default unless detailed health analysis is requested.
- **Dynamic Clinical Insight Generation**: The Doctor Advocacy Report now uses Ollama's native JSON mode to dynamically parse the user's logged symptoms and chat history. It extracts estimated metrics (like NSAID Efficacy and Peak Pain) and builds a cycle-by-cycle timeline with a clinical triage screening summary—fully generated by the local LLM.
- **Dynamic User State**: The dashboard now supports dynamic user naming with local storage persistence to make the greeting personalized.
